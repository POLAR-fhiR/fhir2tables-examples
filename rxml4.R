...
###
save.xmls.in.directory <- function( xmls, path ) {
...
# example
# save.xmls.in.directory( xmls, "data/obsWithEncSubjPat2" )
###


###
# fhir.get.bundle.as.xml.list.from.dir
# reads all downloaded xml files as a list of xml into R
fhir.get.bundle.as.xml.list.from.dir <- function( directory ) {
...
# example:
# xmls2 <- fhir.get.bundle.as.xml.list.from.dir( "data/obsWithEncSubjPat/" )
# rm( xmls2 )
###

###
# fhir.get.page.entries.as.dataframes
# transform an xml page to a dataframe
fhir.get.page.entries.as.dataframes <- function( 
	xml, 
	entries = list( 
		MEDICATION = list(
			tag     = ".//Medication",
...
# example
# d <- fhir.get.page.entries.as.dataframes(
# 	xmls[[ 2 ]],
# 	entries.obs
#  )
# 
	...
	lapply(
		...
		as.data.frame(
			Reduce( 
				...
				lapply(
					xmls,
					function( x ) fhir.get.page.entries.as.dataframes( x, entries )[[ n ]]
				)
			)
		)
	)
